...
from ..game.probing.live_layout import (
    ResolvedLiveLayout,
...
from ..game.probing.live_position import freeze_fast_cursor_lane, log
from ..game.probing.memory_probe import MemReader, auto_pid
from ..game.probing.observation_state import (
    PositionSample,
    ProgressTracker,
    RawRichLane,
    SlowLaneAccumulator,
...
from ..game.runtime.action_sender import ActionSender, open_action_sender
from ..game.runtime.frame_capture import CaptureRegion, FrameCapture
from ..game.runtime.reset_startup import StartupAutomation
from ..game.runtime.window_control import WindowControl
...
def _load_or_resolve_layout_static(
    *,
    pid: int,
    ...
    window: int,
    eps: float,
    layout_discovery_timeout: float,
    live_layout_cache: str | None,
    use_layout_cache: bool,
    discover_rich_layout: bool,
) -> ResolvedLiveLayout:
    if use_layout_cache:
        cache_path = live_layout_cache or default_live_layout_cache_path(pid)
    else:
        cache_path = None
    if cache_path:
        try:
            ...
            if cached_layout.pid == pid and (
                not discover_rich_layout or _layout_has_progress_fields(cached_layout)
            ):
    ...
    if not discover_rich_layout:
        ...
        try:
            layout = resolve_live_layout(
                ...
                window=window,
                eps=eps,
                startup_timeout=layout_discovery_timeout,
...
def _layout_probe_worker(result_queue, kwargs: dict[str, object]) -> None:
    try:
        layout = _load_or_resolve_layout_static(**kwargs)
    except Exception as exc:
        result_queue.put(("error", exc.__class__.__name__, str(exc)))
    else:
        result_queue.put(("ok", layout))
...
class GettingOverItEnv(gym.Env):
    ...
    def __init__(
        ...
        *,
        ...
        startup_input_backend: str = "auto",
        startup_probe_timeout: float = 3.0,
        ...
        self.startup_input_backend = startup_input_backend
        self.startup_probe_timeout = startup_probe_timeout
    ...
    def reset(self, seed: int | None = None, options: dict | None = None):
        ...
        self._reset_trace = {
            ...
            "startup_input_backend": self.startup_input_backend,
            "startup_probe_timeout": self.startup_probe_timeout,
            "startup_probe_attempts": 0,
            "startup_probe_timeouts": 0,
            "old_pid": self.pid,
        }
    ...
    def _load_or_resolve_layout(self, pid: int) -> ResolvedLiveLayout:
        return _load_or_resolve_layout_static(
            pid=pid,
            calibration_samples=self.calibration_samples,
            calibration_interval=self.calibration_interval,
            window=self.window,
            eps=self.eps,
            layout_discovery_timeout=self.layout_discovery_timeout,
            live_layout_cache=self.live_layout_cache,
            use_layout_cache=self.use_layout_cache,
            discover_rich_layout=self.discover_rich_layout,
        )

    def _load_or_resolve_layout_bounded(self, pid: int) -> ResolvedLiveLayout:
        self._reset_trace["startup_probe_attempts"] = (
            int(self._reset_trace.get("startup_probe_attempts", 0)) + 1
        )
        timeout = max(0.1, float(self.startup_probe_timeout))
        ...
        process = context.Process(target=_layout_probe_worker, args=(result_queue, kwargs))
        process.start()
        ...
        if process.is_alive():
            ...
            if process.is_alive():
                process.kill()
                process.join(timeout=1.0)
            self._reset_trace["startup_probe_timeouts"] = (
                int(self._reset_trace.get("startup_probe_timeouts", 0)) + 1
            )
            raise TimeoutError(f"layout probe exceeded {timeout:.3f}s")
        try:
            result = result_queue.get_nowait()
        except queue_mod.Empty as exc:
            raise RuntimeError(f"layout probe exited without a result: {process.exitcode}") from exc
        if result[0] == "ok":
            return result[1]
        _kind, exc_type, message = result
    ...
    def _load_or_resolve_layout_after_reset(self, pid: int) -> ResolvedLiveLayout:
        ...
        while time.monotonic() < deadline:
            ...
            try:
                layout = self._load_or_resolve_layout_bounded(pid)
    ...
    def _drive_startup_until_playable(self, pid: int) -> ResolvedLiveLayout:
        ...
        control = WindowControl(
            ...
            input_backend=self.startup_input_backend,
        )
        ...
        def resolve_layout() -> ResolvedLiveLayout:
            ...
            return self._load_or_resolve_layout_bounded(refreshed_pid)
        ...
        try:
            self._reset_trace["startup_input_backend_resolved"] = control.active_input_backend
        except Exception:
            pass
    ...
    def _capture_startup_full_frame(self) -> np.ndarray:
        ...
        capture = FrameCapture(
            ...
            region=region,
            allow_blank=False,
            prefer_xwd=region is None,
            fallback_xwd_on_blank=region is None,
        )
        try:
